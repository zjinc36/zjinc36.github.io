...
先把这几个核心组件的“职责”和“关联”讲透，是理解六种模式的前提：

| 组件     | 核心职责                                                                  | 关键关联                                                                           |
| -------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Producer | 消息生产者：生产并发送消息到 RabbitMQ                                     | 只能通过 **Channel** 发送消息到 **Exchange**，不直接对接 Queue                     |
| Exchange | 交换机：接收 Producer 的消息，根据“路由规则”转发到 Queue                  | 必须和 Queue 绑定（Binding），绑定规则决定消息转发逻辑（六种模式的核心）           |
| Queue    | 消息队列：存储消息，供 Consumer 消费，是消息的最终存储载体                | 从 Exchange 接收消息，与 Consumer 一一/多对多关联（Consumer 订阅 Queue）           |
| Consumer | 消息消费者：订阅 Queue，从 Queue 拉取/接收消息并处理                      | 只能通过 **Channel** 订阅 Queue，不直接对接 Exchange/Producer                      |
| Channel  | 信道：连接（Connection）内的轻量级通信通道，所有操作（发/收消息）都通过它 | 1 个 Connection 可创建多个 Channel，Producer/Consumer 都通过 Channel 操作 RabbitMQ |

**核心链路**：  
`Producer → Channel → Exchange → (绑定规则) → Queue → Channel → Consumer`  
（所有组件的交互都绕不开 Channel，Exchange 是“路由器”，Queue 是“仓库”）

# 六种工作模式总览

RabbitMQ 六种核心模式本质是 **Exchange 的不同类型 + 绑定规则** 决定的消息转发逻辑，下面用 ASCII 图逐个展示组件交互：

## 1. 简单模式（Simple Queue）
- 核心：无 Exchange（用默认交换机 ""），Producer 直接发消息到指定 Queue
- 组件关系：Producer → Channel → 默认 Exchange → Queue → Channel → Consumer
```
┌───────────┐     ┌───────────┐     ┌───────────┐
│ Producer  │────>│ Channel   │────>│ "" (默认)  │
└───────────┘     └───────────┘     └─────┬─────┘
                                           ...
                                           ▼
┌───────────┐     ┌───────────┐     ┌───────────┐
│ Consumer  │<────│ Channel   │<────│ Queue_A   │
└───────────┘     └───────────┘     └───────────┘
```
- 关键：默认 Exchange 会把“路由键=队列名”的消息直接转发到对应 Queue，是最简单的 1:1 消费。

## 2. 工作队列模式（Work Queue）
- 核心：一个 Queue 被多个 Consumer 订阅，消息轮询/公平分发（避免单 Consumer 过载）
- 组件关系：Producer → Channel → 默认 Exchange → Queue → 多个 Channel → 多个 Consumer
```
┌───────────┐     ┌───────────┐     ┌───────────┐
│ Producer  │────>│ Channel   │────>│ "" (默认) │
└───────────┘     └───────────┘     └─────┬─────┘
...
└───────────┘     └───────────┘
```
- 关键：Queue 是“消息仓库”，多个 Consumer 竞争消费同一个 Queue 的消息，适合任务分发。

## 3. 发布/订阅模式（Fanout Exchange）
- 核心：Fanout（扇出）交换机，把消息广播到所有绑定的 Queue，忽略路由键
- 组件关系：Producer → Channel → Fanout Exchange → 多个 Queue → 多个 Consumer
```
┌───────────┐     ┌───────────┐     ┌────────────┐
│ Producer  │────>│ Channel   │────>│ Fanout_X   │──────────┐
└───────────┘     └───────────┘     └────────────┘          |
                                                            │ (广播，忽略路由键)
                                        ┌───────────────────│
                                        ▼                   ▼ 
┌───────────┐     ┌───────────┐     ┌───────────┐     ┌───────────┐     ┌───────────┐     ┌───────────┐
│ Consumer1 │<────│ Channel1  │<────│ Queue_A   │     │ Queue_B   │<────│ Channel2  │<────│ Consumer2 │
└───────────┘     └───────────┘     └───────────┘     └───────────┘     └───────────┘     └───────────┘
```
- 关键：Fanout Exchange 是“广播器”，只要 Queue 绑定了该 Exchange，就会收到所有消息，实现 1:N 分发。

## 4. 路由模式（Direct Exchange）
- 核心：Direct 交换机，根据“路由键精确匹配”转发消息到绑定的 Queue（绑定键=路由键才转发）
- 组件关系：Producer → Channel → Direct Exchange → 匹配路由键的 Queue → Consumer
```
┌───────────┐     ┌───────────┐     ┌───────────┐
│ Producer  │────>│ Channel   │────>│ Direct_X  │─────────┐ (路由键=error)
└───────────┘     └───────────┘     └───────────┘         |
                                                          │
                                        ┌─────────────────┴──┐
                                        │ (绑定键=info)       │ (绑定键=error)
...
└───────────┘     └───────────┘     └───────────┘     └───────────┘     └───────────┘     └───────────┘
```
- 关键：Direct Exchange 是“精准路由器”，只有 Queue 的绑定键和消息路由键完全一致，才会收到消息，适合按“级别/类型”分发（如 info/error 日志）。

## 5. 主题模式（Topic Exchange）
- 核心：Topic 交换机，支持通配符匹配路由键（`*` 匹配一个单词，`#` 匹配多个单词），是最灵活的模式
- 组件关系：Producer → Channel → Topic Exchange → 匹配通配符的 Queue → Consumer
```
┌───────────┐     ┌───────────┐     ┌───────────┐
│ Producer  │────>│ Channel   │────>│ Topic_X   │────┐ (路由键=order.pay.success)
└───────────┘     └───────────┘     └───────────┘    |
                                                     |
                                        ┌──────────────────┐
                       (绑定键=order.*)  │                  │ (绑定键=order.#)
...
└───────────┘     └───────────┘     └───────────┘     └───────────┘     └───────────┘     └───────────┘
```
- 关键：Topic Exchange 是“模糊路由器”，支持按“层级路由键”匹配（如 order.pay、user.login），你之前问的 100 个 Topic 就是基于这个模式。
