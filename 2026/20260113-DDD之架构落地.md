...
```
- **领域对象的范畴**
  - 领域对象只包含 **实体** 和 **值对象**，它的本质是**承载业务属性和自身行为的“事物”**（比如订单、用户、地址）。
  - 聚合根是**实体的子集**，是有“管理能力”的特殊实体，不是单独的类别。
- **领域服务的定位**
  - 领域服务是**独立于领域对象的元素**，它的本质是**承载“跨对象业务逻辑”的操作**（比如订单支付、计算满减），不是“事物”，而是“动作/服务”。
  - 它和领域对象的关系是 **“服务与被服务”**，而非 **“包含与被包含”**。
- **领域事件（Domain Event）**
  - 就是领域里发生的“值得记录的大事”，目的是解耦不同聚合。
  - 比如：**订单支付成功** 这个事件，不需要订单自己去通知库存扣减、积分增加，而是发布这个事件，库存和积分模块自己监听并处理。
- **聚合（Aggregate）**
  - 它不是一个单独的“对象”，而是一个**业务组合单元**。
  - 比如：订单聚合 = 订单（聚合根） + 订单项（实体） + 收货地址（值对象），这个组合是不可拆分的，保证业务规则一致。
  - 聚合（Aggregate）的核心是业务逻辑上的一致性边界，而它在代码工程结构上的落地表现形式可以是一个目录，见下文
- **仓储（Repository）**
  - 可以理解为**聚合的“专属仓库管理员”**。
  - 外部只需要告诉仓库“保存这个订单聚合”“查询ID为123的订单聚合”，不用关心数据存在MySQL还是Redis。而且仓储只操作聚合根，不会单独保存订单项。
  - 也就是说，仓储是对 “聚合的持久化操作” 做统一抽象，屏蔽底层存储的差异，让领域层只关注 “要操作什么聚合”，而不关心 “数据存在哪、怎么存”
- **应用服务（Application Service）**
  - 是**业务流程的“总指挥”**，但不做具体业务逻辑。
  - 比如：用户下单流程 = 应用服务接收请求 → 调用领域服务校验库存 → 调用仓储保存订单聚合 → 发布订单创建事件。

所以，目录结构可以变成如下

```
├── StartApp.java                # 程序入口（无DDD元素归属，纯启动类）
├── application/                 # 应用层：流程编排，不包含核心业务规则
│   ├── service/                 # 应用服务（核心）
│   │   └── OrderApplicationService.java  # 编排下单流程：调用领域服务+仓储+发布事件
│   ├── command/                 # 命令对象（入参封装）
│   │   └── CreateOrderCommand.java       # 下单请求参数（商品ID、数量、用户ID等）
│   └── dto/                     # 可选：返回结果封装（如OrderResponseDTO）
├── domain/                      # 领域层：核心业务规则、领域模型
│   ├── order/                   # 按聚合划分（订单聚合）
│   │   ├── aggregate/           # 聚合根（核心实体）
│   │   │   └── Order.java       # 订单聚合根（包含订单状态修改、订单项管理等核心行为）
│   │   ├── entity/              # 聚合内非根实体
│   │   │   └── OrderItem.java   # 订单项实体（属于订单聚合，无独立生命周期）
│   │   ├── valueobject/         # 值对象
│   │   │   ├── Money.java       # 金额值对象（金额、币种，重写equals/hashCode）
│   │   │   └── Address.java     # 收货地址值对象
│   │   ├── service/             # 领域服务（跨对象/跨聚合逻辑）
│   │   │   └── OrderDomainService.java   # 订单相关领域服务（如校验库存、计算总价）
│   │   ├── event/               # 领域事件
│   │   │   └── OrderPaidEvent.java       # 订单支付成功事件
│   │   └── repository/          # 仓储接口（领域层定义，基础设施层实现）
│   │       └── OrderRepository.java      # 订单仓储接口（只操作Order聚合根）
│   └── inventory/               # 按聚合划分（库存聚合，示例）
│       ├── aggregate/
│       │   └── Inventory.java   # 库存聚合根
│       └── service/
│           └── InventoryDomainService.java  # 库存领域服务（扣减库存）
└── infrastructure/              # 基础设施层：技术支撑，屏蔽外部依赖
    ├── repository/              # 仓储实现（对接数据库）
    │   └── OrderRepositoryImpl.java       # 实现OrderRepository接口，用MyBatis/ JPA操作数据库
    ├── persistence/             # 可选：数据库映射（如实体类、Mapper接口）
    │   └── mapper/
    │       └── OrderMapper.java
    ├── event/                   # 可选：领域事件发布实现（如基于RabbitMQ）
