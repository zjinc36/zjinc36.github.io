...
> 笔记：
> 
> A.1 是一大摞表，读法其实只有一句：**每条指令占一行，左边是名字和操作数，中间是执行前的栈，右边是执行后的栈**（`...` 表示「栈上其余部分保持原样」）。先记死这套约定：`a`/`b` 是数值，`o`/`p` 是对象引用，`w` 是 long/double，`i`/`j`/`n` 是 int——后面所有表都靠它。
> 
...
> 笔记：
> 
> 常量表是整个 A.1 里最「实用」的一张，因为它直接决定你写 ASM 时怎么压入一个常量。看三档就够了：
> 
...
> 笔记：
> 
> A.2 讲的是「历史遗留」：**`JSR`/`RET` 只在版本 ≤ V1_5 的类里出现**，V1_6 起为了简化新的校验器架构被移除（子例程并非严格必需——编译器不再用它编译 finally 块，改成内联了）。所以今天你基本见不到它们，但分析老类时仍可能撞上。
> 
...
> 笔记：
> 
> A.3 是附录里唯一带代码的小节，重点不在 `CommentAttribute` 的字段，而在三个机制：
> 
...
> 笔记：
> 
> A.5 是全书唯一的实测数据，六条结论里最该带走的：**90% 的变换时间花在类解析和类写出上**（所以优化要从这两处下手）；**「复制常量池」优化带来 15–20% 的加速**；**`COMPUTE_FRAMES` 开销很大 ⇒ 用增量式帧更新**；Tree API 比访问者慢约 25%，正好呼应 1.2.2 节两套 API 的取舍。
> 
