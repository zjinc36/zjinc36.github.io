...
> 笔记：
>
> 先看这一章和第 5 章的关系：这一章是第 5 章的孪生章节，**第 5 章给 Core API 定向后兼容机制，第 10 章给 Tree API 定同一套机制。** 核心前提也照搬——向后兼容不能只靠 ASM 自身保证，需要你遵守几条指南。
>
...
> 笔记：
>
> 先看 10.2 的分界线：**生成器不用管指南，分析器和适配器必须管指南**。原因很直接：生成器从零创建节点，不会碰到未来版本才有的未知特性；分析器和适配器处理的是 `ClassReader.accept()` 填出来的节点，或者自己覆写节点类，才可能遇到这种问题。
>
> 由此得出后面三小节的顺序：先处理自己创建节点的基本规则，再处理继承节点类时的规则，最后把同样的区分推广到 `asm.util`、`asm.commons` 和 `asm.tree.analysis`。
...
> 笔记：
>
> 先看指南 3 的正文：它只有一句话，重点全在它下面的实现里。对照看 4.0 版和 5.0 版的 `ClassNode`：**5.0 版每个 visit 方法开头都先检查 `api < ASM5`**——版本不够就走旧语义，够高才走新语义；新增的 `visitLicense` 在旧版本里直接抛错。
>
...
> 笔记：
>
> 先看指南 4 解决的两难：拿到别人创建的 `ClassNode`，**不能只看 `api` 字段就拒绝**——版本高不代表一定含未知特性；**但又没法测试未知特性**（你此刻根本不知道未来会加什么字段）。所以 `check(api)` 把「该版本不该出现的特性」的检查集中到一处：使用时先 `check(ASM4)`，节点确实含 `license` 这类新特性才抛错。
>
...
> 笔记：
>
> 先看 10.2.2 的关键词是「子类」。只要你提供 `ClassNode` 或类似节点类的子类，第 5 章的指南 1 和指南 2 就回来生效：构造器要声明 ASM 版本，访问者继承要避免被继续覆写破坏语义。
>
> 再看这里给出的常见例外：覆写 `visitEnd()` 的 `MethodNode` 匿名子类虽然没写 `final`，但外部已经无法再继承它，所以指南 2 自动满足。由此得出实际要求：给 `MethodNode(ASM4)` 传版本，或者变换前先 `check(ASM4)`。
...
> 笔记：
>
> 先看 10.2.3 其实只讲一个判断：**你是直接使用，还是要覆写它**。直接使用 `ASMifier`、`Textifier`、`CheckXxxAdapter` 或 `asm.commons` 的类时，可以用无参构造器，等价于使用最新 ASM 版本；一旦要覆写，就按指南 1 和 2，在自己的构造器里用目标 ASM 版本调用 `super(...)`。
>
> 再看 analysis 包的补充：`Analyzer` 处理的是 `MethodNode`，所以在把节点交给它之前，还要先按指南 3 或指南 4 确认这个节点对你的 ASM 版本是安全的。
...
> 笔记：
>
> 先看这段附加代码的对照关系：Core 一边把版本判断写进 `visitSource`，Tree 一边把版本判断分成两条路径——自己创建节点时用 `new ClassNode(Opcodes.ASM4)`，拿到别人节点时用 `other.check(Opcodes.ASM4)`。
>
> 由此得出整章的收束：不管是 Core 还是 Tree，版本参数都不是装饰，它是在告诉 ASM「我的代码只理解到这个版本」，好让未知特性尽早失败。
